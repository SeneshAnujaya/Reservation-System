...
//
//    private static final String INSERT_SQL =
//            "INSERT INTO reservations (guest_name, email, phone, room_type, check_in, check_out, status, room_type_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
//
//    private static final String SELECT_ALL_SQL =
//            "SELECT * FROM reservations";
//
...
//            "SELECT * FROM reservations";
    ...
    private static final String SELECT_ALL_SQL =
            "SELECT r.*, rm.room_type AS room_type_name FROM reservations r " +
                    "JOIN rooms rm ON r.room_type_id = rm.room_id";

//    private static final String SELECT_BY_ID_SQL =
//            "SELECT * FROM reservations WHERE id = ?";

    private static final String SELECT_BY_ID_SQL =
            "SELECT r.*, rm.room_type AS room_type_name " +
                    "FROM reservations r " +
                    "JOIN rooms rm ON r.room_type_id = rm.room_id " +
                    "WHERE r.id = ?";

    private static final String UPDATE_SQL =
            "UPDATE reservations SET guest_name=?, email=?, phone=?, room_type=?, check_in=?, check_out=?, status=?, room_type_id=? WHERE id=?";
    ...
    public List<Reservation> selectAllReservations() throws Exception {
        ...
        try (Connection connection = getConnection();
            ...
            while (rs.next()) {
                ...
                r.setEmail(rs.getString("email"));
                r.setPhone(rs.getString("phone"));
                r.setRoomType(rs.getString("room_type_name"));
                r.setCheckIn(rs.getDate("check_in"));
                r.setCheckOut(rs.getDate("check_out"));
                r.setStatus(rs.getString("status"));
    ...
    public Reservation selectReservation(int id) {
        ...
        try (Connection connection = getConnection();
            ...
            if (rs.next()) {
                ...
                reservation.setEmail(rs.getString("email"));
                reservation.setPhone(rs.getString("phone"));
                reservation.setRoomType(rs.getString("room_type_name"));
                reservation.setCheckIn(rs.getDate("check_in"));
                reservation.setCheckOut(rs.getDate("check_out"));
                reservation.setStatus(rs.getString("status"));
